...
const Chat = () => {
  const { data, loading, error, getAllData, setShowProfile } = useContext(AppContest); // getting data from contest api
      ...
      {hide ? (
        <div className={style.profiles}>
          ...
          {data &&
            data.map((el) => (
              <div key={el.id}>
                <div className={style.maping_div} onClick={() => alert(el.id)}>
